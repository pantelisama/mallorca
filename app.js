...
const cards=daySpots.map(s=>{const i=spots.indexOf(s);const photo=s.photo||"";return "<article class='spot-card' data-spot-index='"+i+"' tabindex='0' role='button'>"+(photo?"<img loading='lazy' src='"+escAttr(photo)+"' alt='"+escAttr(s.n)+"'>":"")+"<div class='spot-info'><div class='spot-cat'>"+categories[s.cat].icon+" "+categories[s.cat].label+"</div><h3>"+s.n+"</h3>"+(s.rating?"<div class='spot-rating'>★★★★★ <strong>"+s.rating+"</strong> · "+(s.reviews||0).toLocaleString()+" reviews</div>":"")+(s.type?"<p class='spot-type'>"+s.type+"</p>":"")+"<p>"+s.d+"</p></div></article>";}).join("");
